...
class TransUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # a) CNN feature maps
        f0 = self.backbone_layers[0](x)
        f1 = self.backbone_layers[1](f0)
        f2 = self.backbone_layers[2](f1)
        f3 = self.backbone_layers[3](f2)
        # f4 = self.backbone_layers[4](f3)
        # print(f0.shape)  # (B, 64, H/2, W/2)
        # print(f1.shape)  # (B, 64, H/4, W/4)
        # print(f2.shape)  # (B, 256, H/4, W/4)
        # print(f3.shape)  # (B, 512, H/8, W/8)
        # print(f4.shape)  # (B, 1024, H/16, W/16)

        if x.shape[2:] != (224, 224):
            # Resize input to 224x224 if needed
            x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
        # b) Prepare patches from f4
        vit_out = self.transformer(pixel_values=x, output_hidden_states=True)
        # hs = vit_out.hidden_states  # list of layer outputs
        # Choose only last hidden state (wo CLS token)
        out = self.decoder(vit_out.last_hidden_state[:, 1:, :], [f0, f2, f3])
        return out
